...
$storeChargeQ = "SELECT COUNT(total) AS storechg_count, IFNULL(ROUND(-SUM(d.total),2), 0) AS storechg_total
...
$houseChargeQ = "SELECT COUNT(total) AS housechg_count, IFNULL(ROUND(-SUM(d.total),2), 0) AS housechg_total
	FROM dtransactions AS d
	WHERE d.trans_subtype = 'MI'
	AND card_no != 9999
	AND d.trans_status <> 'X'
...
$trimetQ = "SELECT IFNULL(SUM(quantity), 0) AS trimet_count, IFNULL(ROUND(SUM(total),2), 0) AS trimet_sales
	FROM dtransactions
	WHERE date(datetime) = '".$db_date."'
	AND department = 40 
	AND trans_status <> 'X'
...
$stampsQ = "SELECT IFNULL(SUM(quantity), 0) AS stamp_count, IFNULL(ROUND(SUM(total),2), 0) AS stamp_sales
	FROM dtransactions
	WHERE date(datetime) = '".$db_date."' 
	AND trans_status <> 'X'
	AND emp_no <> 9999
...
$sistersQ = "SELECT IFNULL(SUM(quantity), 0) AS sisters_count, IFNULL(ROUND(SUM(total),2), 0) AS sisters_sales
	FROM dtransactions
	WHERE date(datetime) = '".$db_date."'
	AND trans_status <> 'X'
	AND emp_no <> 9999
...
$returnsQ = "SELECT IFNULL(SUM(quantity), 0) AS returns_count, IFNULL(ROUND(sum(l.total),2), 0) as returns_total
