...
{
    // Regular expression to validate that the time has been entered in the valid format of HH:MM xM
    time_test = /^([1-9]|1[0-2]|0[1-9]){1}(:[0-5][0-9]\s?[aApP][mM]){1}$/;

    if (time_test.test(start_time))
    {
        // If the start time is PM, then add 12 hours to the time.
        ...
        // If the end time is PM, then add 12 hours to the time.
