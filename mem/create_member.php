...
if (isset($_POST['submitted'])) { // If the form has been submitted, check the data and create the record.
	...
	if ((isset($_POST['ps'])) && (isset($_POST['ss']))) { // Both Primary and Secondary. 
		...
		if ((empty($_POST['ps_phone_number'])) || (empty($_POST['ss_phone_number']))){
			
			$errors[] = 'You left one or both of their phone numbers blank.';
			
		} else {
			$pspn = escape_data($_POST['ps_phone_number']); // Store the phone numbers.
			$sspn = escape_data($_POST['ss_phone_number']); // Store the phone numbers.
		}
		if ((empty($_POST['ps_email'])) || (empty($_POST['ss_email']))){
			
			$errors[] = 'You left one or both of their emails blank.';
			
		} else {
			$psem = escape_data($_POST['ps_email']); // Store the email addresses.
			$ssem = escape_data($_POST['ss_email']); // Store the email addresses.
		}
		...
		if ((empty($_POST['ps_discount'])) || (empty($_POST['ss_discount']) && $_POST['ps_discount'] != 0)) {
			
			$errors[] = 'You left one or both of their discounts blank.';
			
		} elseif (($_POST['ps_discount'] > 0) || ($_POST['ss_discount'] > 15)) {
		...
		if (empty($_POST['ps_phone_number'])) {	
			$errors[] = 'You left their phone number blank.';
		} else {
			$pspn = escape_data($_POST['ps_phone_number']); // Store the phone number.
		}
		if (empty($_POST['ps_email'])) {
			$errors[] = 'You left their phone number blank.';
		} else {
			$psem = escape_data($_POST['ps_email']); // Store the phone number.
		}
		...
		if (empty($_POST['ps_discount']) && $_POST['ps_discount'] != 0) {
			
			$errors[] = 'You left their discount blank.';
			
		} elseif ($_POST['ps_discount'] > 0) {
	...
	if (empty($errors)) {
		$ps = "($cn, 1, '$psfn', '$psln', '$pspn', '$psem', $psWriteCheck, $psd, $psmemtype, '$psType', $ps_staff, $pscharge, $pslimit)";
		if ($num_records == 2) {$ss = ", ($cn, 2, '$ssfn', '$ssln', '$sspn', '$ssem', $ssWriteCheck, $ssd, $ssmemtype, '$ssType', $ssstaff, $sscharge, $sslimit)";}
		elseif ($num_records == 1) {$ss = '';}
		$query = "INSERT INTO custdata (CardNo, personNum, FirstName, LastName, phoneNo, email, WriteChecks, discount, memType, Type, staff, ChargeOk, MemDiscountLimit) VALUES " . $ps . $ss;
echo $query . '~' . $ps . '~' . $ss;
		$result = @mysql_query($query);
...
// Show the form.
	...
	<p>Phone Number: <input type="text" name="ps_phone_number" size="10" maxlength="15" /></p>
	<p>E-mail Address: <input type="text" name="ps_email" size="20" maxlength="45" /></p>
	';
	...
	<p><input type="hidden" name="ps_discount" size="3" maxlength="2" value="0" /></p>
