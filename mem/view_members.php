...
require_once ($_SERVER["DOCUMENT_ROOT"].'/src/mysql_connect.php'); // Connect to the DB.
...
if (isset($_GET['sort'])) { // If a non-default sort has been chosen.
	...
	switch ($_GET['sort']) {
		...
		case 'pna':
		$order_by = 'phoneNo ASC';
		$link4 = "{$_SERVER['PHP_SELF']}?sort=pnd";
		break;
		
		case 'pna':
		$order_by = 'phoneNo DESC';
		$link4 = "{$_SERVER['PHP_SELF']}?sort=pna";
		break;
		
		case 'ema':
		$order_by = 'email ASC';
		$link5 = "{$_SERVER['PHP_SELF']}?sort=emd";
		break;
