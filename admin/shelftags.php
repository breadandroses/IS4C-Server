 ...
 */
require($_SERVER["DOCUMENT_ROOT"].'/src/fpdf/fpdf.php');
...
$testQ = "select if(u.brand IS NULL,'',substring(u.brand,1,12)) as brand,  
		...
		if(u.description IS NULL, substring(p.description,1,23),substring(u.description,1,23)) as description,   
 ...
 */
   $price = 'M $' . number_format($row['memberPrice'], 2) . ' R $' . number_format($row['normal_price'], 2);
 ...
 */
   $newUPC = $upc . $check;
   $pdf->UPC_A($barLeft,$barTop,str_pad(substr($upc, 0, -1), 12, 0, STR_PAD_LEFT), 7);
/**
