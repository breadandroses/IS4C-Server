...
include_once($_SERVER["DOCUMENT_ROOT"]."/define.conf");
		...
		if ($lane_continue == 1) {
			...
			if (mysql_query($opdata_truncate, $lane_conn)) {
				if (mysql_query($opdata_insert, $lane_conn)) {
					...
					echo "<br><font color='#800000' face=helvetica size=-1>Unable to synchronize ".$lane_num."</font><div>".$opdata_insert."</div>";
				}
			}
		}
...
//	openlog("is4c_connect", LOG_PID | LOG_PERROR, LOG_LOCAL0);
	...
	exec('echo "truncate table products;" | ' . "mysql -u ".$_SESSION["laneUser"]." ".$lanepass." -h ".$lane." ".$opDB." 2>&1", $aResult);

	$sync =  "mysqldump -u ".$_SESSION["mUser"]." ".$serverpass." -t ".$opDB." ".$table
		  ." | mysql -u ".$_SESSION["laneUser"]." ".$lanepass." -h ".$lane." ".$opDB." 2>&1";
